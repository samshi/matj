function createMessageArea(f) {
  ...
  P.confirm = $.C(P, {
    ...
    T:
      P.input_message.T_ +
      (P.input_message.H_ + P.input_message.PD_ * 2 - 32) / 2,
    W: 80,
    H: 30,
    LH: 30,
    F: 16,
  ...
  let t =
    P.input_message.T_ + P.input_message.H_ + P.input_message.PD_ * 2 + 10;
  P.message_area = $.C(P, {
    L: 0,
    T: t,
    PDB: 10,
  ...
  P.setMessage = async function (action, message) {
    ...
    switch (action) {
      ...
      case "N": //down
        P.wait++;
        message_res = await INNER.matj.message(
          message_file,
          name,
          action,
          message
  ...
  P.showMessage = function (all_message = "") {
    ...
    let keys = all_message.match(
      /(\=_)?(\w{5}\-){10}\w\w\w\^\d+\^\w+\^[A-Z]\^/g
    );
    if (!keys) {
      P.message_area.I("");
      return;
    ...
    for (let j = 0, l = keys.length; j < l; j++) {
      let [principalId, time, auther, action] = keys[j]
        .replace(/^\=_/, "")
        .slice(0, -1)
        .split("^");
      ...
      let pos =
        j == l - 1 ? all_message.length : all_message.indexOf(keys[j + 1]);
    ...
    for (let i = 0, l = all.length; i < l; i++) {
      let { index, principalId, time, auther, action, msg, U, N, D } = all[i];
      ...
      s += `<img class="avatar" onmouseover="P_CHANNEL.large(event)" onmouseout="P_CHANNEL.large()" src="${P_CANVAS.principalToAvatar(
        principalId
      )}"/>`;
      s += `</td>`;
      s += `<td class="sizetime" style="text-align:left;">${auther}<br/>${$.getDatetime(
        "dort",
        time
      )}</td>`;
      s += `<td class="message">${msg.replace(/\n/g, "<br/>")}</td>`;
      s += `<td class="channel_svg" onclick="P_MESSAGE.click(this)" onmouseover="P_MESSAGE.over(this, event)" onmouseout="P_MESSAGE.out(this)" data-act="U">${
        SVG.U
      }<br><span>${P_MESSAGE.totalCnt(U)}</span></td>`;
      s += `<td class="channel_svg" onclick="P_MESSAGE.click(this)" onmouseover="P_MESSAGE.over(this, event)" onmouseout="P_MESSAGE.out(this)" data-act="N">${
        SVG.N
      }<br><span>${P_MESSAGE.totalCnt(N)}</span></td>`;
      s += `<td class="channel_svg" onclick="P_MESSAGE.click(this)" onmouseover="P_MESSAGE.over(this, event)" onmouseout="P_MESSAGE.out(this)" data-act="D">${
        SVG.D
      }<br><span>${P_MESSAGE.totalCnt(D)}</span></td>`;
  ...
  P.over = function (node, e) {
    ...
    for (let principalId in P.all[i][act]) {
      s +=
        `<div style="display:inline-block; padding:2px 5px;text-align:center;"><img class="avatar" src="${P_CANVAS.principalToAvatar(
          principalId
        )}"/>` +
        "<span>" +
        P.all[i][act][principalId][0].auther +
        "<br>" +
        (P.sum(P.all[i][act][principalId]) || "") +
        "</span></div>";
    }
