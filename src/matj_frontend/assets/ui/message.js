function createMessageArea(f){
  ...
  P.showMessage    = function(all_message = ''){
    ...
    for(let j = 0, l = keys.length; j < l; j++){
      ...
      P.checkExist(all[i], action, {})
      P.checkExist(all[i][action], principalId, [])

      all[i][action][principalId].push(obj)
    }

    all.sort((a, b) => {
      let kau = P.total(a.U)
      let kan = P.total(a.N)
      let kat = P.timePow(a.time)
      let kbu = P.total(b.U)
      let kbn = P.total(b.N)
      let kbt = P.timePow(b.time)

      return kau - kan + kat < kbu - kbn + kbt ? 1 : -1
    })
    ...
    for(let i = 0, l = all.length; i < l; i++){
      ...
      s += `<td class="channel_svg" onclick="P_MESSAGE.click(this)" onmouseover="P_MESSAGE.over(this)" onmouseout="P_MESSAGE.out(this)" data-act="U">${SVG.U}<br><span>${P_MESSAGE.totalCnt(U)}</span></td>`;
      s += `<td class="channel_svg" onclick="P_MESSAGE.click(this)" onmouseover="P_MESSAGE.over(this)" onmouseout="P_MESSAGE.out(this)" data-act="N">${SVG.N}<br><span>${P_MESSAGE.totalCnt(N)}</span></td>`;
      s += `<td class="channel_svg" onclick="P_MESSAGE.click(this)" onmouseover="P_MESSAGE.over(this)" onmouseout="P_MESSAGE.out(this)" data-act="D">${SVG.D}<br><span>${P_MESSAGE.totalCnt(D)}</span></td>`;
  ...
  P.checkExist = function(obj, item, def){
    if(!obj[item]){
      obj[item] = def
    }
  }

  P.timePow = function(n){
    return Math.pow(10, Math.max(0, 3 - (Date.now() - n) / 87600000))
  }
  ...
  P.click    = async function(node){
    ...
    P.checkExist(P.all[i], act, {})
    P.checkExist(P.all[i][act], principalId, [])

    let obj = P.all[i][act] || {}
    obj[principalId].push(res)
